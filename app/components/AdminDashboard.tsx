...
export function AdminDashboard() {
  ...
  const filtered = useMemo(() => customers.filter((customer) => `${customer.name} ${customer.org} ${customer.plan}`.toLowerCase().includes(query.toLowerCase()) && (!activeOnly || customer.status === "Active")), [activeOnly, query]);
  ...
  return (
    <>
      ...
      <section className="portal-card audit-card" id="audit"><div className="card-heading"><div><span>ACCOUNT ACTIVITY</span><h2>Audit log</h2></div><span className="preview-label">Today</span></div><div className="audit-list"><p><b>Subscription viewed</b><span>Doug Simmons · just now</span></p><p><b>Support queue reviewed</b><span>Doug Simmons · 24 minutes ago</span></p><p><b>Prospect status updated</b><span>System preview · 1 hour ago</span></p></div></section>
